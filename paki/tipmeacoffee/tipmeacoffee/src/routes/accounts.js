...
const getAccountPub = (username) => {
    return new Promise((res, rej) => {
        breej.getAccount(username, function (error, account) {
            if (error) rej(error);
            if (!account) rej();
            if (account.pub) res(account.pub);
        })
    })
}
const validateToken = async (username, token) => {
    ...
    try {
        var decrypted = CryptoJS.AES.decrypt(token, msgkey, {
            iv: iv
        });
        return breej.privToPub(decrypted.toString(CryptoJS.enc.Utf8)) === await getAccountPub(username);
    } catch (err) {
        return false;
    }
}

const msgkey = process.env.msgKey;
const iv = process.env.breezval;
MongoClient.connect("mongodb://localhost:27017/besocial", function (err, client) {
    ...
    try {
        ...
        breej.getAccount(username, function (error, account) {
            ...
            try {
                ...
                res.send({
                    error: true,
                    message: 'Password (privkey) seems incorrect'
                });
                return
            }
            if (account.pub !== pubKey) {
                res.send({
                    error: true,
                    message: 'Password (privkey) validation fails'
                });
            } else {
                var encrypted = CryptoJS.AES.encrypt(key, msgkey, {
                    iv: iv
                });
                var token = encrypted.toString();
                res.cookie('breeze_username', username, {
...
async function signup(req, res) {
    try {
        ...
        if (!inputName.match(allowed_name)) {
            res.send({
                error: true,
                message: 'Only alphanumeric usernames allowed (all lowercase)'
            });
            return false;
        };
        if (inputName.length < 5) {
            res.send({
                error: true,
                message: 'Username length should not be less than 5'
            });
            return false;
        };
            ...
            if (!accounts || accounts.length === 0) {
                db.collection('users').findOne({
                    ...
                    if (user) {
                        res.send({
                            error: true,
                            message: 'phew... Email is already in use'
                        });
                    } else {
                ...
                res.send({
                    error: true,
                    message: 'phew.. Username already exist'
                });
            }
        })
    } catch (err) {
    ...
    try {
        ...
        if (token && await validateToken(req.cookies.breeze_username, req.cookies.token)) {
            ...
            db.collection('users').findOne({
                ...
                } else if (result && result.isvarified == true) {
                    v_token = 'verified', username = result.username.toLowerCase()
                } else {
                    v_token = 'invalid', username = ''
                }
                loguser = "";
                res.render('common/verify', {
                    vtoken: v_token,
                    user_id: username,
                    trendingTags: nTags,
                    loguser: loguser,
...
async function keygen(req, res) {
    try {
        let post = req.body;
        let allowed_name = /^[0-9a-z]+$/;
        if (!post.name.match(allowed_name)) {
            res.send({
                error: true,
                message: 'Only alphanumeric usernames allowed (all lowercase)'
            });
            ...
            if (!accounts || accounts.length === 0) {
                ...
                db.collection('users').findOne({
                    ...
                    if (result) {
                        ...
                        let privAc = process.env.privKey;
                        let signedTx = breej.sign(privAc, 'breeze', newTx)
                        ...
                        res.send({
                            error: true,
                            message: 'phew.. Not a verified user'
                        });
                    }
                })
            } else {
...
async function verifyMail(rAddress, message) {
    let transporter = nodemailer.createTransport({
        host: process.env.TRANSMAIL_HOST,
        port: 465,
        secure: true,
        auth: {
            user: process.env.TRANSMAIL_KEY,
            pass: process.env.TRANSMAIL_PASS,
        },
        tls: {
    ...
    let info = await transporter.sendMail({
        from: 'Tip Me A Coffee <verify@tipmeacoffee.com>',
        to: rAddress,
